...
- Examples:
  - ✅ `./dc.sh up -d` (start services)
  - ✅ `./dc.sh exec web python manage.py runserver` (run in container)
  - ✅ `python manage.py makemigrations` (create migrations locally)
  - ❌ `docker compose -f deploy/docker-compose.yml up -d` (use `./dc.sh` instead)
...
```
ERROR (red)     → Code crashed, must fix before commit
WARNING (yellow) → Potential issues, investigate
INFO (blue)     → Normal operations, can ignore
DEBUG (gray)    → Detailed info, useful for debugging
...
❌ Use `docker compose -f deploy/docker-compose.yml` directly (use `./dc.sh` instead)  
❌ Create migrations with `./dc.sh` (use local `python manage.py makemigrations`)  
❌ **Commit code without testing:** Always run `./dc.sh exec web pytest` first  
❌ **Ignore service errors:** Check `./dc.sh logs web --tail=100` for errors  
❌ **Skip log review:** Always check for ERROR and WARNING in logs  
...
# If anything shows ERROR or FAILED → FIX BEFORE COMMITTING
```

**Key Testing Rules:**
- ✅ **Always test with Docker:** Code must work in `./dc.sh` environment
- ✅ **Check logs first:** `./dc.sh logs web` catches 80% of issues
- ✅ **Never ignore errors:** Even WARNING level needs investigation
- ✅ **Test edge cases:** Not just happy path
- ✅ **Verify database:** Migrations must apply cleanly
- ✅ **Check API responses:** 200 status ≠ correct data
- ✅ **Test in browser:** Frontend rendering matters
- ✅ **Review logs after testing:** Ensure no background errors
